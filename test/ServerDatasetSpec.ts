                        ...
                        if(zip.file("index.htm")== undefined){ reject(true)}else{
                            ...
                            for(let file in zip.files){
                                ...
                                if(zip.file(file)!=null){
                                    ...
                                    zip.file(file).async("string").then(function (data:string) {
                                        let pnode = parse5.parse(data);
                                        promises.push( that.parsehtmlRoom(pnode));
                                    })
                                }
                            }
                            ...
                            Promise.all(promises).then(function (data) {
                                ...
                                for(let room of data){
                                    //     building = new RegExp(room.rooms_href,"/room/")
                                }
